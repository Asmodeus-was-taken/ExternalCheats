...
inline auto __fastcall setup_window() -> void
{
	...
	MyWnd = CreateWindowExA(0, wClass.lpszClassName, ("---X---X---X---X---X---X---X---X---X---X---X---X---X---X"), WS_POPUP | WS_VISIBLE, GameRect.left - 2.f, GameRect.top - 2.f, globals.width + 2.f, globals.height + 2.f, 0, 0, 0, 0);
...
inline auto setup_directx(HWND hWnd) -> int
{
	...
	io.Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\Ruda-Bold.ttf", 50);
...
void RenderMenu()
{
	...
	if (globals.menu)
	{
		...
		step();
		ImGui::Begin(("Playboy.cc"), (bool*)0);
		{
			
			ImGui::CenterText("News: ");
			ImGui::Text("______________________\n");
			ImGui::Text("Version 1.0:            ");
			ImGui::Text(" New R6 External...");
			ImGui::Text("Added Esp & Misc");
			ImGui::Text("UD Since Release");
...

	{
		...
		ImGui::CenterText("ESP");
		Imgui::Spacing();
		Imgui::Spacing();
		ImGui::Checkbox("Enable Visuals", &Var.enableEsp);
			if (Var.enableEsp)
			{
				ImGui::Checkbox("Player Box", &Var.cornerbox);
				if (Var.cornerbox)
				{
					ImGui::Combo(("##BoxType"), &car.BoxType, ("Corner\0Box\0Weirdbox"));
				}
				ImGui::Checkbox("Player Filled Box", &_var::fillBox);
				ImGui::Checkbox("Player Health", &_var::healthbar);
				ImGui::Checkbox("Draw Lines", &_var::Lines);
				ImGui::Checkbox("Draw Names", &_var::Name);
			}
	}
	ImGui::EndChild();
	ImGui::Spacing();
	...
	{
		ImGui::SetCursorPosY(ImGui::GetCursorPosY() + 75);
		ImGui::CenterText("Misc");
		Imgui::Spacing();
		Imgui::Spacing();
		ImGui::Checkbox("Crosshair", &_var::gaycrosshair); 
		ImGui::Checkbox("No Recoil", &_var::recoil);
		ImGui::SliderFloat("Recoil Amount", &_var::recoilAmount, ImVec2(100.0f));
		ImGui::Checkbox("No Sway", &_var::sway);
		ImGui::SliderFloat("Sway Amount", &_var::swayAmount, ImVec2(100.0f));
		ImGui::Checkbox("Custom Night", &_var::nicenight);
		ImGui::Checkbox("Team Check", &_var::team);
	}
	ImGui::EndChild();
